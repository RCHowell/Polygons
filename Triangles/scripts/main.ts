...
class Controller {
  ...
  handleClick(event: any): void {
    if (this.pointCount == 3) this.pointCount = 0;
  ...
  changeShapeColor(colors: string[]): void {
    this.shapes[this.shapes.length - 1].changeColor(colors[this.colorCount]);
...
document.body.onkeyup = function(e){
  ...
  if(e.keyCode == 32) controller.changeShapeColor(palette.list);
}
